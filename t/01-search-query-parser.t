...
SKIP: {
    ...
    eval { require Search::Query::Parser; };
    if ($@) {
        plan skip_all => "Search::Query::Parser required for testing";
    }
...

    {
        ...
        # override base method to grab sort_cache
        sub make_matcher {
            my ( $self, %args ) = @_;
            my $child_matcher
                = $self->get_child_compiler->make_matcher(%args);
            return unless $child_matcher;
            my $sort_reader
                = $args{reader}->obtain("Lucy::Index::SortReader");
            my $sort_cache = $sort_reader->fetch_sort_cache('option');
            return $self->matcher_class->new(
                child      => $child_matcher,
